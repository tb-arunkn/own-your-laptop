...
import { User, resetUserPassword, toggleUserStatus, deleteUser } from '../services/api';
import { Users, Mail, Calendar, Shield, Key, CheckCircle, AlertCircle, UserX, Trash2, UserCheck } from 'lucide-react';
              ...
              <tr key={user.id} className={`hover:bg-gray-50 ${user.isActive === false ? 'opacity-60 bg-gray-50' : ''}`}>
                ...
                <td className="px-6 py-4 whitespace-nowrap text-sm font-medium">
                  <div className="flex items-center gap-2 flex-wrap">
                    ...
                    {actionError && (togglingStatus === user.id || deleting === user.id) && (
                      <div className="flex items-center gap-1 text-red-600 text-xs">
                        <AlertCircle className="h-3 w-3" />
                        {actionError}
                      </div>
